...
// Trontop local patch: opt-in native device recovery and typed diagnostics.
...
pub struct WgpuConfiguration {
    /// Trontop opt-in: recreate a failed device off the UI thread and replay managed
    /// textures. Applications using native textures/custom GPU callbacks must rebuild
    /// their own resources; this option is intended for managed-texture-only UIs.
    pub recover_device: bool,
    /// Closed categories only; no driver error strings or application data.
    pub on_renderer_event: Arc<dyn Fn(RendererEvent) + Send + Sync>,
