...
pub struct Painter {
    // Trontop local patch: the native window/context survive device replacement.
    #[cfg(not(target_arch = "wasm32"))]
    recovery: crate::recovery::DeviceRecovery,
    #[cfg(not(target_arch = "wasm32"))]
    texture_replay: crate::recovery::TextureReplay,
    recovery_needs_present: bool,
    context: Context,
...
impl Painter {
    ...
    pub async fn new(
        ...
        Self {
            recovery_needs_present: false,
            #[cfg(not(target_arch = "wasm32"))]
            recovery: Default::default(),
            #[cfg(not(target_arch = "wasm32"))]
            texture_replay: Default::default(),
            context,
            config,
            options,
    ...
    fn recreate_surface(
        ...
        let (width, height, resizing) = (old_state.width, old_state.height, old_state.resizing);
        // Retain the old entry on failure so its dimensions and retry flag survive.
        let surface = self.instance.create_surface(Arc::clone(window))?;
        self.install_surface(surface, viewport_id, width, height, resizing);
        Ok(())
    }
    ...
    fn resize_and_generate_depth_texture_view_and_msaa_view(
        ...
        #[cfg(not(target_arch = "wasm32"))]
        if self.config.recover_device && self.recovery.needed() {
    ...
    pub fn paint_and_update_textures(
        ...
        #[cfg(not(target_arch = "wasm32"))]
        if self.config.recover_device {
            self.texture_replay.apply(textures_delta);
            if self.recovery.needed() {
                let context = self.context.clone();
                let surface = self
                    .surfaces
                    .get(&viewport_id)
                    .map(|s| Arc::clone(&s.surface));
                if let Some(state) = self.recovery.poll_or_start(
                    &self.config,
                    &self.instance,
                    surface,
                    self.options,
                    ...
                    self.texture_replay.invalidate();
                    self.render_state = Some(state);
                    self.depth_texture_view.clear();
                    self.msaa_texture_view.clear();
                    self.screen_capture_state = None;
                    for surface in self.surfaces.values_mut() {
                        // A driver reset can invalidate BOTH device and surface.
                        // Recreate against each existing window on its next paint;
                        // do not configure the new device against a known-lost surface.
                        surface.needs_recreate = true;
                    }
                    self.recovery_needs_present = true;
                } else {
                    // Polling is non-blocking; the worker also wakes us on completion.
                    // No unconditional timer on the healthy path.
                    self.context
            ...
            && let Err(err) = self.recreate_surface(viewport_id, window)
        {
            ...
            #[cfg(not(target_arch = "wasm32"))]
            if self.config.recover_device {
                // The frame's delta is already consumed by egui. Keep a full
                // replay pending, including deltas arriving during later retries.
                self.texture_replay.invalidate();
            }
            self.context
        ...
        #[cfg(not(target_arch = "wasm32"))]
        let replayed =
            self.config.recover_device && self.texture_replay.upload_if_pending(render_state);
        ...
        if render_state.renderer.read().upload_failed() {
            #[cfg(not(target_arch = "wasm32"))]
            if self.config.recover_device {
                self.recovery.fail_upload(&self.config);
        ...
        #[cfg(not(target_arch = "wasm32"))]
        if self.recovery_needs_present && !self.recovery.needed() {
            self.recovery_needs_present = false;
            (self.config.on_renderer_event)(crate::RendererEvent::Recovered);
            self.context.request_repaint();
        }
