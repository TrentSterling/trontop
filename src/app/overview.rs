...
impl TrontopApp {
            ...
            .show(ui, |ui| {
                ...
                ui.horizontal_wrapped(|ui| {
                    ...
                    match (&bridge.status, fresh) {
                        (crate::specs::Value::Known(providers), true) => {
                        ...
                        (crate::specs::Value::Unavailable(_), _) => {
    ...
    /// CPU and motherboard readings from the specs sensor bridge, with their
    /// provider and sensor names. Nothing here reads hardware directly.
    fn bridge_sensor_cards(&self, ui: &mut egui::Ui, now: Instant) {
        ...
        if !cores.is_empty() {
            let text = cores
                .iter()
                .map(|r| r.unit.format(r.value))
                .collect::<Vec<_>>()
                .join("  ");
            widgets::detail_row(ui, "CPU core temperatures", &text, t);
        }
        let note = match &bridge.status {
            Value::Known(providers) => format!(
                "Read-only values published by {providers}. {} readings in total; every one is listed on System > Sensor Sources. ACPI thermal zones are never shown as CPU temperature.",
                bridge.readings.len()
            ),
            Value::Unavailable(reason) => format!(
                "Unavailable: {reason}. ACPI thermal zones are not assumed to be CPU package readings."
            ),
        };
